...
export default function Home() {
  ...
  if (loading) {
    return <div className="auth-wrap"><div className="card auth-card">Loading portal…</div></div>
  }
    ...
    return (
      <div className="auth-wrap">
        <div className="card auth-card">
          ...
          <button type="button" className="btn btn-secondary" style={{ marginTop: 16 }} onClick={() => refreshProfile(userId)}>Try Again</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="auth-wrap">
        <div className="card auth-card">
          ...
          <p className="subtitle">Your profile and schedule were submitted. An Admin or Manager must approve the account before portal access is enabled.</p>
          <div className="row"><span>Requested role</span><strong>{prettyRole(profile.requested_role)}</strong></div>
          <button className="btn btn-secondary" style={{ marginTop: 16, width: '100%' }} onClick={() => supabase.auth.signOut()}>Sign out</button>
        </div>
      </div>
    )
  }
  ...
  if (profile.status !== 'active') {
    return (
      <div className="auth-wrap">
        <div className="card auth-card">
          <h1 className="title">Account unavailable</h1>
          <p className="subtitle">Your account status is {profile.status}.</p>
          <button className="btn btn-secondary" style={{ width: '100%' }} onClick={() => supabase.auth.signOut()}>Sign out</button>
        </div>
      </div>
    )
  }
      ...
      <main className="main">
        <div className="topbar">
          ...
          <div className="actions">
            <span className="pill position-pill">{prettyRole(profile.role)}</span>
